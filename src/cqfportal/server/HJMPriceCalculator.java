...
public class HJMPriceCalculator {
	private HJMFactorData calib;
	private HJMVolatilities vols;	
	private int numPaths;
	
	private static Logger logger = 
			Logger.getLogger(HJMPriceCalculator.class.getName()); 
			
	public HJMPriceCalculator(HJMFactorData c, HJMVolatilities v, int nPaths) {
		calib = c;
		vols = v;		
		numPaths = nPaths;
	}
	...
	private double getZCBPriceSinglePath(ZCB zcb) {
		...
		YieldCurveSet ycs = new HJMCurveModeller().getCurveSet(calib, vols, zcb.maturity);		
		double df = getDF(ycs);				
		return df;						
	}
	...
	private double getCapletPriceSinglePath(Caplet caplet) {
		...
		YieldCurveSet ycs = new HJMCurveModeller().getCurveSet(calib, vols, caplet.maturity);				
